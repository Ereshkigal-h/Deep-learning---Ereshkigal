...
def initialize(sequence_length, batch_size, filepath=None, sheet_name=None, skiprows=0, skipfooter=0):
    ...
    holidays_status=scaler2.fit_transform(data_array[:,0].reshape(-1, 1))
    person_number=scaler1.fit_transform(data_array[:, 1].reshape(-1, 1))
    normalized_data_array = np.hstack((holidays_status,person_number))
    ...
    return train_iter, test_iter, scaler1
